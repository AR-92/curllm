...
# Test what's actually happening with help --verbose

set -euo pipefail

# Test directory
TEST_DIR="/tmp/curllm_test_actual"
mkdir -p "$TEST_DIR"
...
echo "Testing what's actually happening with help --verbose..."

# Create a debug version that shows what's being checked
cat > /tmp/debug_actual_help.sh << 'EOF'
#!/usr/bin/env bash

# Source the libraries from the test directory
SCRIPT_DIR="/tmp/curllm_test_actual"
...
show_help() {
    echo "SHOWING STANDARD HELP"
}

show_verbose_help() {
    echo "SHOWING VERBOSE HELP"
}
...
main() {
    ...
    echo "DEBUG: parsed_args = '$parsed_args'"
    
    # Check if we just need to show help or version
    if [[ "$parsed_args" == "help_verbose|||" ]]; then
        echo "DEBUG: Calling show_verbose_help"
        show_verbose_help
        exit 0
    fi
    
    if [[ "$parsed_args" == "help|||" ]] || [[ "$parsed_args" == "|||" ]]; then
        echo "DEBUG: Calling show_help"
        show_help
        exit 0
    fi
    
    if [[ "$parsed_args" == "version|||" ]]; then
        echo "DEBUG: Calling show_version"
        show_version
        exit 0
    fi
    
    echo "DEBUG: Would continue with normal processing"
}
...
chmod +x /tmp/debug_actual_help.sh

echo "=== Testing: help --verbose ==="
/tmp/debug_actual_help.sh help --verbose

echo ""
echo "=== Testing: help ==="
/tmp/debug_actual_help.sh help

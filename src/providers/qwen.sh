...
qwen_chat_completion() {
    local prompt="$1"
    local model="${2:-qwen-turbo}"
    
    # Check if we're in mock mode
    if [[ "${MOCK_MODE:-false}" == "true" ]]; then
        echo "This is a mock response from Qwen for prompt: $prompt"
        return 0
    fi
    ...
    if [[ "$http_code" -ne 200 ]]; then
        echo "Error: Qwen API request failed with status $http_code" >&2
        echo "$body" >&2
        return 1
    fi
    
    # Extract and return the response text
    echo "$body" | jq -r '.output.text'
}